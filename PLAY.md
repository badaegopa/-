...
광고 1회 = **별 +1 · 걸음 +2**. 둘 다 상한이 없고 판을 넘어 보존됩니다.
기본 걸음은 매 판 **18보로 리셋**되므로, 광고로 모은 걸음은 별도 창고에 쌓였다가
막혔을 때 꺼내 쓰는 구조입니다.

별은 **숲에 들어가기 전 0~6개를 골라 소모**합니다. 고른 만큼만 배치되고
찾지 못해도 돌아오지 않으므로, 광고 시청이 곧바로 소비되지 않고
**사용자가 소비 시점과 양을 정하는** 형태가 됩니다.

리워드 광고를 두 곳에서 부릅니다 — 시작 화면·결과 화면의 적립용,
그리고 걸음이 막힌 실패 화면의 이어가기용. 둘 다 같은
`Play.showRewardedAd()` 를 지나므로 광고 단위는 하나면 됩니다.
