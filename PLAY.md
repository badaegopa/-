...
| `angel` | 천사의 축복 | 초특급 행운상자를 엶 |
| `first_win` | 첫 당첨 | 발행한 티켓이 5등 안에 듦 |
...
**상자 정리(재도전) 루프.** 여섯을 채우면 먼저 **자리만** 공개하고(🗿 토템 · ✦ 천사 ·
⭐ 행운의 상자, 약 2.6초), 정리 화면이 뜹니다. 여기서 상자를 버리면
그만큼 다시 찾아야 하고, 걸음이 모자라면 실패 화면을 거쳐 광고로 이어갑니다.
**"이대로 확정"** 을 눌러야 지도가 다시 펼쳐지며 숫자가 **5초** 공개되고 결과로 넘어갑니다.
숫자를 마지막까지 미루는 이유는 두 가지입니다 — 재도전의 의미를 지키기 위해서,
그리고 **번호 공개가 곧 세션의 절정**이 되도록 배치하기 위해서입니다.
버리는 행위 자체에는 어떤 대가도 붙이지 않았습니다 — 강제 시청이 아니라
**사용자가 원해서 다시 걷는** 흐름이어야 정책상으로도, 체감상으로도 안전합니다.
(광고를 보지 않아도 남은 걸음으로 계속할 수 있고, 언제든 "이대로 확정"이 가능합니다.)
